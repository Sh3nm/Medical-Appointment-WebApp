...
import { LogOut, User, Mail, Calendar, Stethoscope, Save, ArrowLeft } from "lucide-react";
...
export default function DoctorProfile() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const errorMsg =
        ...
        "Gagal menghapus akun. (Catatan: akun dokter mungkin dibatasi untuk dihapus.)";
      setError(errorMsg);
    } finally {
      setDeleting(false);
    }
  ...
  const handleUpdate = async () => {
    ...
    try {
      // Update name and password through /users/me
      ...
      // Update specialization through /doctors/:id if changed
      if (specialization && specialization !== profile?.specialization) {
        try {
          await api.patch(`/doctors/${profile?.id}`, {
            specialization: specialization,
          });
        } catch (err: any) {
          console.error("Failed to update specialization:", err);
          // Continue with other updates even if specialization fails
        }
      }
      ...
      if (Object.keys(updateData).length > 0) {
        ...
        setProfile({ ...updatedData, specialization: specialization || updatedData.specialization });
        setName(updatedData.name || "");
        setOldPassword("");
        setNewPassword("");
        setConfirmPassword("");
      } else if (specialization && specialization !== profile?.specialization) {
        // Only specialization was updated
        setProfile({ ...profile, specialization });
      } else {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  placeholder="Contoh: Dokter Umum, Kardiologi, Pediatri"
                />
              </div>
